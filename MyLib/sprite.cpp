...
Sprite::Sprite(const wchar_t* texture)
{
	ID3D11Device* orgDevice = FRAMEWORK->GetDevice();
	//①頂点情報のセット
	vertex virtices[] = {
...
SpriteBatch::SpriteBatch(const wchar_t* _fileName, size_t _maxInstance, DirectX::XMFLOAT2 _cutOutSize )
{
	...
	//シェーダーのロード
	hr = create_vs_from_cso(device, "Data/Shader/sprite_batch_vs.cso", &vertexShader, &inputLayout, layout, ARRAYSIZE(layout));
	assert(SUCCEEDED(hr) && "vertexShagerの読み込み中にこけました　by SpriteBatch");
	hr = create_ps_from_cso(device, "Data/Shader/sprite_batch_ps.cso", &pixelShader);
