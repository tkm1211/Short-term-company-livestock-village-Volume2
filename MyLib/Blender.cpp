...
Blender::Blender(ID3D11Device* _device)
{
	...
	//NONE
	{
		...
		assert(SUCCEEDED(hr) && "NONEのブレンドステート生成でerror");
	}
	//ALPHA
	{
		blendDesc.AlphaToCoverageEnable = FALSE;
		...
		assert(SUCCEEDED(hr) && "ALPHAのブレンドステート生成でerror");
	}
	//ADD
	{
		blendDesc.AlphaToCoverageEnable = FALSE;
		...
		assert(SUCCEEDED(hr) && "ADDのブレンドステート生成でerror");
	}
	//SUBTRACT
	{
		blendDesc.AlphaToCoverageEnable = FALSE;
		...
		assert(SUCCEEDED(hr) && "SUBTRACTのブレンドステート生成でerror");
	}
	//MULTIPLY
	{
		blendDesc.AlphaToCoverageEnable = FALSE;
		...
		assert(SUCCEEDED(hr) && "MULTIPLYのブレンドステート生成でerror");
	}
}
